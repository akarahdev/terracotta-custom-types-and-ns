...
import { ConditionDefinition, DefinitionType, FunctionDefinition, FunctionCallExtraInfo } from "./definition.ts";
...
function getSchemaNamespaceArgument(
    ...
    if (!(value instanceof TangibleValue)) {
        ctx.reportError(callNode, "Namespace reflection requires a schema-backed namespace value");
        return null;
    }
    let type = value.getType(ctx.types);
    if (!type.matches(Type.namespace)) {
        ctx.reportError(callNode, `Namespace reflection requires a namespace, got '${type.name}'`);
        return null;
    }
    let namespace = (type.data as NamespaceTypeData).namespace;
    if (!(namespace instanceof SourceNamespace) || !namespace.supportsRuntimeReflection) {
        ctx.reportError(callNode, "Namespace reflection requires a namespace with a schema");
        return null;
    }
    return value;
}

function namespaceValueType(args: Expression[], types: EvaluationContext["types"]): Type {
    let argument = args[0];
...
const namespaceArgumentSignature = [{
    params: [{name: "namespace", type: Type.any, optional: false, plural: false}],
}];
...
const getValues: FunctionDefinition = {
    ...
    compile(args, namedArgs, ctx, callNode, extraInfo = {}): [CodeValue, CodeBlock[]] {
        ...
        let namespace = getSchemaNamespaceArgument(args, callNode, ctx);
        if (namespace == null) return [new EmptyValue(callNode), []];
        let output = ctx.tvp.newTempVar(Type.list(namespaceValueType(callNode.args.elements, ctx.types)));
        return [output, [new ActionBlock(DFCodeblockName.SET_VARIABLE, {
...
const hasMember: ConditionDefinition = {
    ...
    compileIf(args, namedArgs, ctx, callNode, extraInfo = {}): [CodeValue, CodeBlock[]] {
        validateArguments(args, callNode, this.signatures, ctx);
        let namespace = getSchemaNamespaceArgument(args, callNode, ctx);
        let key = args[1];
        if (namespace == null || !(key instanceof TangibleValue)) return [new EmptyValue(callNode), []];
        return [new EmptyValue(callNode), [new ActionBlock(DFCodeblockName.IF_VARIABLE, {
            action: "DictHasKey",
            args: [namespace, key],
        })]];
    },
