...
import { FunctionSchemaTypeExpression, NamespaceSchemaFieldStatement, NamespaceSchemaFunctionStatement, NamespaceSchemaStatement, NamespaceStatement, NamespaceVariableStatement } from "../../ast/statement.ts";
import type { Definition, FunctionDefinition, NamespaceVariableDefinition } from "./definition.ts";
...
export function getSourceNamespaceMemberBackendName(path: readonly string[], member: string): string {
    return `${SOURCE_NAMESPACE_MANGLE_PREFIX}${encodeSourceNamespacePath([...path, member])}`;
}

export function getSourceNamespaceDictionaryBackendName(path: readonly string[]): string {
    return `${SOURCE_NAMESPACE_DICTIONARY_MANGLE_PREFIX}${encodeSourceNamespacePath(path)}`;
}
...
export class SourceNamespace extends Namespace {
    ...
    getDynamicFunctionDefinition: ((member?: string | number) => FunctionDefinition | null) | null = null;
...
export interface SourceNamespaceVariableDefinition extends NamespaceVariableDefinition {
...
export function isSourceNamespaceVariableDefinition(definition: unknown): definition is SourceNamespaceVariableDefinition {
    return (
...
export interface SourceNamespaceFunctionSchema {
    ...
    declaration: NamespaceSchemaStatement | NamespaceSchemaFieldStatement | NamespaceSchemaFunctionStatement,
...
export interface SourceNamespaceValueShapeField extends SourceNamespaceShapeFieldBase {
    kind: "value",
    type: Type,
}

export interface SourceNamespaceFunctionShapeField extends SourceNamespaceShapeFieldBase {
    kind: "function",
    definition: FunctionDefinition,
    processOnly?: boolean,
}

export interface SourceNamespaceNestedShapeField extends SourceNamespaceShapeFieldBase {
    kind: "namespace",
