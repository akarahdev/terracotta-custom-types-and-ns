...
import type { SourceNamespace } from "./namespace/sourceNamespace.ts";
...
/**
 * A schema-backed source namespace has no DF value of its own.  Instead, its
 * path is carried until a concrete member is requested, at which point the
 * compiler prefixes it with the source-namespace backing-variable family.
 */
export class RuntimeNamespaceValue extends InternalValue {
  private type: Type;

  constructor(
    public namespace: SourceNamespace,
    /** Path after the backing-name prefix, including dynamic `%var(...)` segments. */
    public source: PCode[],
    astNode?: ASTNode,
  ) {
    super(astNode);
...
export class FunctionValue extends InternalValue {
  constructor(
    ...
    astNode?: ASTNode,
    public runtimeNamespaceAccess?: NonNullable<
...
export class ItemValue extends TangibleValue {
  ...
  templateForm() {
    return {
      ...
      "data": {
        "item":
          `{count:${this.count}b,DF_NBT:${this.dfNbt},id:"${this.id}",components:${
            this.nbt ?? "{}"
          }}`,
      },
    };
...
export class VariableValue extends TangibleValue {
  ...
  getVarId(): VariableId {
    return VariableId.get(
      this.scope,
      typeof this.name == "string" ? this.name : this.name.join(""),
    );
  }
  ...
  getType(typeProcessor: TypeProcessor): Type {
    ...
    return frame.getVariableType(
      this.getVarId(),
      this.astNode?.startPos ?? Infinity,
    );
  }
  ...
  templateForm() {
    ...
    switch (this.scope) {
      case VariableScope.GLOBAL:
        scope = "unsaved";
        break;
      case VariableScope.SAVED:
        scope = "saved";
        break;
      case VariableScope.LOCAL:
        scope = "local";
        break;
      case VariableScope.LINE:
        scope = "line";
        break;
    }
    return {
    ...
    return {
      ...
      "data": {
        ...
        "default_value": this.defaultValue != null
          ? this.defaultValue.templateForm()
          : undefined,
        "plural": this.plural,
